// Shared catalogue search over the raw scraped SQLite category files.
// Used by the authed vendor tool (/portal/catalogue) AND the public search
// landing (/search/catalogue) — one query path, two front doors.
...
export const CATEGORIES = ["watches", "shoes"];
...
// qs: { q, category, stock, size, brand, source, sort, price_min, price_max, page, limit }
export async function searchCatalogue(qs = {}) {
  const q = (qs.q || "").toString().trim().toLowerCase();
  const cat = (qs.category || "").toString();
  const stock = (qs.stock || "").toString();
  const size = (qs.size || "").toString().trim();
  const brand = (qs.brand || "").toString().trim().toLowerCase();
  const source = (qs.source || "").toString();
  const sort = (qs.sort || "").toString();
  const priceMin = parseFloat(qs.price_min);
  const priceMax = parseFloat(qs.price_max);
  const limit = Math.min(parseInt(qs.limit, 10) || 24, 60);
  ...
  const buildWhere = () => {
    ...
    if (size) { where.push(`sizeName LIKE ?`); params.push(`%"${size}"%`); }
  ...
  return { page, count: pageRows.length, hasMore: merged.length > offset + limit, results: pageRows };
}
